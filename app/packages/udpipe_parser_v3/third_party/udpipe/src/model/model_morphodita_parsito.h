...
class model_morphodita_parsito : public model {
 public:
  ...
  // Tomato V3 exposes the parser's normalized transition log score for
  // health diagnostics. The public UDPipe API discards this value even though
  // Parsito already computes it. A higher value (closer to zero) indicates a
  // more probable transition sequence; it is calibrated by task-level tests,
  // never interpreted as an absolute probability.
  bool parse_with_cost(sentence& s, const string& options, string& error,
                       double* cost) const {
    return parse(s, options, error, cost);
  }

  ...
  static List<TextGenerationTurn> _chapterPlanPromptTurns({
    ...
    return [
      const TextGenerationTurn(
        role: 'system',
        content:
            'Create a picture-book scene plan from non-dialogue visual prose. Return strict minified JSON only.',
      ),
      TextGenerationTurn(
        role: 'user',
        content: [
          ...
          '- Chapter text is the source prose. Base chapterDescription and sceneDescription on its visible details, but remove direct dialogue, song lyrics, shouted text, inner thoughts, and dialogue-meaning summaries.',
          ...
          // Keep scene planning mechanical: remove speech/thought first, then
          // split only when the continuous story scene changes.
          '- Before splitting scenes, mentally delete all quoted speech, dialogue meaning, song text, shouted text, and inner thoughts; use only the remaining non-dialogue source prose to decide scene boundaries.',
          '- If deleting speech/thought leaves no new non-dialogue place, time, participant group, activity, or action-result change between two candidate boundaries, those sentence slots must stay in the same scene.',
          ...
          '- Start a new scene only when the continuous story scene changes: new place/time, participant group change, or a concrete non-dialogue action moves the story into a new situation or activity.',
          ...
          '- Dialogue, song, shout, and inner-thought sentences are coverage anchors inside the surrounding story scene; remove speech/thought content and keep only source-prose visible action or reaction.',
          '- Do not write substitute dialogue-summary words in chapterDescription or sceneDescription: exchange, conversation, discuss, debate, ask, answer, question, reply, remark, riddle, argue, claim, mean, say, said, offer.',
          ...
          '- sceneDescription: describe only the visible scene, action, objects, location, composition, emotion, and visual change from non-dialogue source prose.',
          '- sceneDescription must not contain dialogue text, quoted speech, lyrics, displayed text copied from speech, or inner thoughts from the source prose.',

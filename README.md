...
## 最新版本 · v1.8.1 声音设置与播放体验（2026-09-14）

补丁版本，聚焦发音人选择、设置页布局和播放切页体验，不改变已发布文章的分句边界。

- 火山 `seed-tts-2.0` 补齐教学 / 有声书 / 客服类英语声，并可只显示英文声音。
- 「语音合成」与发音人列表提到设置页顶部；「保存声音」跟在下方；云服务 / 歌曲保存改为醒目主按钮。
- 听力全屏、歌曲全屏与歌曲内联切页跟随录制 `pageTransition` 设置；听力全屏从当前选中句开始。

> 已发布文章继续使用持久化句槽，不会因本补丁自动重分。

[查看 v1.8.1 发布说明](docs/releases/v1.8.1.md) ·
[查看变更记录](docs/change_log.md) ·
[查看 v1.8.0 分句说明](docs/releases/v1.8.0.md)

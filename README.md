...
应用直接调用云 REST API，不依赖本地后端或中间服务。当前项目重点覆盖书籍/章节管理、章节连续听力、绘本和 Suno 歌曲创作、逐句跟读、发音评分、AI 对话练习，以及本地学习记录管理。

## 核心功能

- 文章导入与本地分句处理，保存为书籍下的章节
- 书库、书籍详情与章节播放器：按书籍组织章节，支持听力/歌曲模式
- 创作中心：集中管理绘本组图、Suno 歌曲、字幕时间轴和视频导出
...
- 绘本 prompt policy 升级为 `chapter_storyboard_group_v2` / `picture_book_chapter_plan_v2`：强调 full-frame 16:9 连续绘本场景、当前分镜动作匹配和自然场景构图，不再在图片提示词中要求为 App 字幕留白。
...
- 歌曲生成入口已收敛为 Suno 网页自动化和本地歌曲版本库，MiniMax API、默认来源设置、mock 和测试入口已清理。
- 听力歌曲弹窗拆为“播放 / 生成”页签：已下载歌曲按本地版本播放；Suno 会按当前歌词记录检测到的完整歌曲链接，只下载缺失 `songUrl`，已下载过的同一链接不会重复落盘，再次生成时会重新让 Suno 根据歌词生成风格。
- 本地歌曲版本新增默认播放标记，Web UI 可把指定 Suno 版本设为默认，Native 播放时优先选择默认版本。
- Suno 歌曲新增字幕时间轴：用 BigASR 词级时间对齐 App 提交给 Suno 的原歌词，播放歌曲时按进度切换英文字幕，并可先生成字幕再导出歌曲版绘本视频。
- Suno 下载音频和 metadata 会迁移到程序运行目录 `suno-music/`，设置页会忽略 `.tmp` 或系统临时目录，Windows 发布脚本保留该目录避免重新发布丢失歌曲资产。

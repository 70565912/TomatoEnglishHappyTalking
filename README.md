...
## 最新版本 · v1.8.2 短句合并与分句优化（2026-09-23）

- 不足4词先与邻接内容合并，允许同一文章内跨段合并短标题和诗行；恰好4词可保留。
- 合并后只求解一次，继续遵循原长度分档和30词硬上限。
- 三本书154章回放中短分句308→0；代表章节同机AOT中位耗时下降29.7%。

> 已保存文章继续读取原有句槽，不会自动重新分句。

[查看 v1.8.2 发布说明](docs/releases/v1.8.2.md) ·
[查看变更记录](docs/change_log.md) ·
[查看分句验证记录](docs/read_aloud_short_merge_validation_20260922.md)

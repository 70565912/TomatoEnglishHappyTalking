...
- 绘本链路使用 `ChapterStoryOutlineService` 复用章节结构化分镜提纲（`chapter_story_outline_v1`）作为对话提纲和规划上下文；组图生成由 v4 审核计划驱动，默认最多 12 段覆盖完整章节。
...
- 绘本提示词审核新增“保存提示词”动作：`pictureBook.savePromptReview` 只保存书籍简介、`storyBrief`、`chapterBrief`、`scenes[]` 和 `groupPrompt`，不调用图片 API、不删除旧图；只有 `pictureBook.confirmPromptReview` 才进入组图生成。
...
绘本提示词中的书籍角色描述会随章节审核逐步完善：可识别的经典名著会让文本模型基于公开常识列出主要递归角色；无法识别的普通书不会硬编全书角色。本章出现但书籍描述尚未包含的角色会作为章节角色补充进入审核草稿，用户保存或确认后再合并到书籍简介，供后续章节保持外观一致。最终组图 prompt 会压缩上下文和单图描述，保证 12 张场景仍保留完整 Image 条目且尽量贴合图片接口提示词长度限制。

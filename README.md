...
完成后发现其实也是一个相当不错的英语学习内容（电子绘本/视频）的全自动化生产工具。
跟阿里云的商务申请开发试用时，又发现这个工具其实也是一个相当不错的AI平台横向对比效果的工具。
它生成出来的产物还能精准用于别家优秀大模型的数据标注蒸馏训练。

应用直接调用云 REST API，不依赖本地后端或中间服务。当前项目重点覆盖书籍/章节管理、章节连续听力、绘本、阿里云百聆（Fun-Music）/ Suno 歌曲创作、逐句跟读、发音评分、AI 对话练习，以及本地学习记录管理。

## 核心功能

- 文章导入与本地分句处理，保存为书籍下的章节
- 书库、书籍详情与章节播放器：按书籍组织章节，支持听力/歌曲模式
- 创作中心：集中管理绘本组图、阿里云百聆（Fun-Music）/ Suno 歌曲、字幕时间轴和视频导出
...
- 云平台选择现在是平台级开关：阿里云百炼平台负责文本、万相绘本组图、CosyVoice TTS、Qwen-ASR；火山引擎平台负责方舟文本、Seedream 绘本组图、Doubao TTS、BigASR，任一平台失败时不会自动回退到另一平台。
- 聊天与跟读共用逐句播放状态 UI：等待播放时先显示动态省略号；真正开始播放后显示文本并尾随省略号；失败时显示错误图标；每句都支持重播。
- 绘本链路由 v4 审核计划驱动，场景按故事段落切分，最多 12 段覆盖完整章节；对话练习提纲由 `ChatChapterGuideService` 单独生成紧凑覆盖点。
- 绘本图片生成切换为顺序组图模式：`PictureBookService` 通过 `PictureBookImageService` 按当前云平台分流，阿里云走万相异步连续组图，火山走 Seedream 顺序组图；重试入口会重新打开提示词审核并在确认后重建整章组图。
- 绘本 prompt policy 升级为 `picture_book_group_prompt_scene_description_v2` / `picture_book_chapter_scene_plan_v2`：保存章节后先打开提示词审核，用户确认 `groupPrompt` 后才提交 Seedream 顺序组图；旧 series Bible、角色卡、参考图、`negativePrompt` 和字幕留白字段已下线。
- 绘本提示词审核新增“保存提示词”动作：`pictureBook.savePromptReview` 只保存书籍简介、`chapterDescription`、`scenes[].sceneDescription` 和 `groupPrompt`，不调用图片 API、不删除旧图；只有 `pictureBook.confirmPromptReview` 才进入组图生成。
...
- 练习中心章节列表可折叠/展开；章节行提供“听力 / 跟读 / 对话”三个入口，其中“听力”直接进入书籍播放器的 listening 模式。
...
常规 Windows / Android 构建脚本会自动执行 `npm ci` 或 `npm install`，再执行 `npm run build`，并把最新 Web UI 产物写入 `app/assets/web/` 后打包进 EXE/APK。Windows 脚本会在 `node_modules/.tomato-package-lock.sha256` 记录依赖状态，依赖未变化时跳过重复安装；如果本地 `node_modules` 被占用，会改用临时目录构建并同步产物。
...
| 阿里云百聆（Fun-Music） | 歌曲音频生成 | `https://dashscope.aliyuncs.com/api/v1/services/audio/music/generation` |
...
绘本提示词中的书籍角色描述由书籍简介和章节审核内容决定；可识别的经典名著可让文本模型基于公开常识列出主要递归角色，无法识别的普通书不要硬编全书角色。最终组图 prompt 按审核后的书籍简介、章节描述和分镜描述完整提交，不再压缩上下文或单图描述。

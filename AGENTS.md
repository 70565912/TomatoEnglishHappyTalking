...
- 整章绘本审核支持手工设置**场景数量**（不做单行增删）：确认后调用 `pictureBook.refreshPromptReview` 并带 `targetSceneCount`，由文本 AI 按该数量匹配已有 `articles.sentences` 区间并生成描述；确认组图仍整章删旧重生。QA 可用 `pictureBook.replaceChapterPlan` 整表重设 `chapterDescription` + `scenes[]`（含句子区间），不调文本 AI、不直接出图。

...
- 整章绘本审核支持手工设置**场景数量**（不做单行增删）：确认后调用 `pictureBook.refreshPromptReview` 并带 `targetSceneCount`，由文本 AI 按该数量匹配已有 `articles.sentences` 区间并生成描述；确认组图仍整章删旧重生。QA 可用 `pictureBook.replaceChapterPlan` 整表重设 `chapterDescription` + `scenes[]`（含句子区间），不调文本 AI、不直接出图。场景数量标签、数字输入框和「按此数量匹配分镜」按钮在审核框标题行保持单行横排，Windows 窄窗口不得换行或溢出。
...
- 分句：`app/lib/services/nlp_service.dart` 与 `web_ui/src/sentenceSplitter.ts`。这是 **朗读块（read-aloud chunk）** 生成器，不是语言学“真分句”；目标块长约 10–20 词，**硬上限 30 词**，避免过短碎片与悬挂尾句，并与外部配音平台逐句字幕前端限制一致。规则必须通用，不得为单篇文章写死特例。E10/E11/E12 等只作回归样本。句末 `.!?` 后无空白时也可收束新句（如粘连的 `Name."Next`），且不会把下一句开头引号吞进上一句；切分优先 `,/;/:/—` 与 connector（含 `which` / `before` / `after` 等）；未闭合双引号内短引语仍跳过逗号，从约 10 词起允许逗号；若仍无标点可选，才回退词界硬切，且硬切前会再回退到窗口内最近标点。保存后 `articles.sentences` 为听力/跟读/歌曲字幕/绘本的持久化边界；改算法后须重建文章才会更新句库。

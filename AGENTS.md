...
- `pictureBook.pageImage` 支持 `variant: "full" | "display" | "thumbnail"`；创作中心和书籍封面应优先请求 `thumbnail`（`640x360`，`picture_book_thumbnails`），不要在列表页一次性把整章原图作为 data URI 加载。监听/跟读/对话内嵌场景图（`.picture-book-scene`）只请求 `display`（`1280x720`，`picture_book_display`，本地缩放缓存原图，不重新调用生成 API）；把原始 `2560x1440` 图直接塞进这个仅 ~700-1120px 宽、`object-fit: cover` 的内嵌容器会在 Windows WebView2 部分 GPU 上触发降采样纹理花屏（坑位见 `docs/build-and-release-pitfalls.md`）。只有全屏播放（`FullscreenListeningPlayer`/`FullscreenSongPlayer`，各自显式 `useEnsurePictureBookPageImage({ imageVariant: 'full' })`）和导出/创作中心大图预览才请求 `full`，并通过 bridge 返回 data URI（不要把缓存目录 `file://` 路径直接交给 WebView `<img>`）。创作中心绘本组图缩略图可点击预览原图：预览层通过 portal 固定在视口中央，遮罩阻挡其它操作，仅点击大图关闭；预览层不要用 `backdrop-filter` 叠在大图上（Windows WebView2 易花屏），可先把 data URI 转成 Blob URL 再显示；预览请求 full 时不要覆盖列表中的 thumbnail URI。

...
- Suno 网页自动化在 `app/lib/features/web_shell/suno/`（`SunoAutomationController` + WebView 胶水）执行：每次生成都只用当前歌词填表；如果 `Styles` 折叠，必须先点击 `Styles` 折叠头展开到能看到 `Styles` 工具栏魔法棒，再清空旧 `Styles` value 并点击蓝色 `Personalize style prompt to match your taste` 魔法棒，等待 Suno 根据歌词写入真实 `Styles` value。不要把 `More Options` 当成 `Styles` 展开入口。保存下来的风格只作为本次歌曲 metadata，不再回填或作为下载筛选条件；不要把默认 placeholder、`Refresh recommended styles` 或 `Add style:` 推荐标签当成自动风格结果。
- Suno 填表只能在 `https://suno.com/create` 执行；**Lyrics 为 Lexical 内联编辑器**（非 textarea），自动化经 `SunoWebViewPaste` **自动 CDP 粘贴 + probe**（见 `docs/suno_lexical_lyrics_editor.md`）；Styles 仍为 textarea。字段定位应排除 Search / Current page / Song Title / Enhance lyrics、Cowriter prompt 等工具输入框，但不要用 textarea 正文参与工具框判断，避免歌词里的普通单词 `search` 误伤真正的 Lyrics / Styles。
- **Suno Lexical 填表改动后必须 live 验证**：修改 `app/lib/features/web_shell/suno/`（含 `SunoWebViewPaste`、`suno_automation_controller`、`suno_web_scripts`）或 `tools/qa_suno_fill_*.mjs` 后，同一轮会话内须先 `flutter test test/suno_lexical_lyrics_test.dart`，再 **rebuild 并跑** `npm run qa:suno-fill-loop`（等价 `node tools/qa_suno_fill_loop.mjs --rebuild --attempts 3 --articleId 84`）。单元测试不能代替 Windows App + Suno WebView 联调；失败时自动重试/排查，不要只改代码不跑程序、也不要把「待用户手动验证」当作完成。通过条件见 `docs/suno_lexical_lyrics_editor.md`「Live 验证」。
- **Suno 注入 JS 转义**：`_lexicalLyricsHelperJs` 用 Dart `r'''` raw 字符串，正则写 `\d` / `\s` / `\/`（**禁止** `\\d` `\\s*\\/`）；改 `suno_web_scripts.dart` 后 `flutter test test/suno_lexical_lyrics_test.dart` 必须通过 **`node --check`** 语法组。典型故障：页面上 Lyrics 可见但日志 `focus_failed` + 空 `data`。规则见 `docs/suno_lexical_lyrics_editor.md`「Dart 嵌入 JS 转义坑」。
...
- 文章歌曲版本归属 `articleId`：`listening.songState` 必须列出该文全部本地有效版本，**禁止**用当前 `lyricsHash` / `contentHash` 过滤已落盘 cache。`lyricsHash` 仅用于新 cache 条目的 dedup、metadata 记录与 Suno **新**下载时的详情页歌词核对；删除/更新必须就地改对应 cache 行并清理 mp3/metadata，禁止整包 rewrite 到当前 hash 产生孤儿文件。规则见 `docs/article_song_version_retention.md`。
- Suno 歌曲下载唯一性按 `songUrl` 与同歌词组判断：`versions` 可保留旧 `stylePrompt` / `styleKey` metadata 但不再按风格分组；`detectedSongUrls` / `downloadComplete` 描述**当前歌词 hash 对应组**的链接落盘状态。重新检测下载时只下载缺失链接，不要重复下载已存在的同一 `songUrl`；用户再次选择生成新歌时仍应重新生成 Suno 风格并进入 Create 确认流程。
...
- 检测下载在 Suno Library 必须**广召回 + DOM 自上而下**打开未下载的 `/song/` 行；Create 提交后的 downloading 阶段共用 `SunoCompletionPolicy` + `SunoCreateBatch`，不得在 batch pending 或 Library 仍有候选时 `complete`。
...
- 章节规划 AI 使用完整原文作为可见细节来源，但 `chapterDescription` / `scenes[].sceneDescription` 必须排除直接引语、对话内容、歌词/喊话文本和内心独白原句；只保留动作、物体、地点、姿态、场景状态、人物关系、情绪表现等图片可表达信息。不要新增本地对话剔除器，也不要为去对话额外增加一次文本 AI 调用；通过 `picture_book_chapter_scene_plan_v2` 的同一次规划 prompt 约束完成。分镜策略仍按 visual story beat，不因 dialogue turns 拆分。

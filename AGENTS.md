...
- `pictureBook.pageImage` 支持 `variant: "full" | "thumbnail"`；创作中心和书籍封面应优先请求 `thumbnail`，缩略图持久缓存在 `picture_book_thumbnails`，不要在列表页一次性把整章原图作为 data URI 加载。听力播放、全屏和导出需要原图时再请求 full/original，并通过 bridge 返回 data URI（不要把缓存目录 `file://` 路径直接交给 WebView `<img>`；坑位见 `docs/build-and-release-pitfalls.md`）。创作中心绘本组图缩略图可点击预览原图：预览层通过 portal 固定在视口中央，遮罩阻挡其它操作，仅点击大图关闭；预览层不要用 `backdrop-filter` 叠在大图上（Windows WebView2 易花屏），可先把 data URI 转成 Blob URL 再显示；预览请求 full 时不要覆盖列表中的 thumbnail URI。

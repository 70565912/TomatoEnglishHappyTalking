...
describe('App', () => {
  ...
  it('shows split cloud capabilities and song provider tabs', async () => {
    ...
    expect(screen.getByRole('tab', { name: 'Suno（系统浏览器）' })).toHaveAttribute('aria-selected', 'true');
  ...
  it('starts Suno song generation from the creation center', async () => {
      ...
      callHandler: vi.fn(async (_handlerName: string, message: Record<string, unknown>): Promise<BridgeResponse> => {
        ...
        if (type === 'listening.songGenerate') {
          return ok(message.id, type, {
            ...
            source: 'suno',
            manualActionMessage:
              '整篇歌词（58 字）已复制到剪贴板，并已在系统浏览器打开 Suno Create。请在浏览器中登录、粘贴歌词、设置风格并 Create；在 Suno 下载 MP3 后，回到创作中心点击「导入本地音乐」添加歌曲版本。',
          });
        }
        return ok(message.id, type, {});
      }),
    ...
    fireEvent.click(generateButton);
    await confirmDialogAction('确认生成歌曲', '继续', /系统浏览器打开 Suno Create/);
    ...
    expect(
      await screen.findByText(/已复制到剪贴板，并已在系统浏览器打开 Suno Create/),
    ).toBeInTheDocument();
  });
  ...
  it('imports external audio songs from the creation center', async () => {
    ...
    expect(screen.queryByRole('button', { name: /检测下载/ })).not.toBeInTheDocument();
  ...
  it('submits Suno song generation with manual browser guidance', async () => {
      ...
      callHandler: vi.fn(async (_handlerName: string, message: Record<string, unknown>): Promise<BridgeResponse> => {
        ...
        if (type === 'listening.songGenerate') {
          ...
          return ok(message.id, type, {
            ...
            source: 'suno',
            manualActionMessage:
              '整篇歌词（28 字）已复制到剪贴板，并已在系统浏览器打开 Suno Create。请在浏览器中登录、粘贴歌词、设置风格并 Create；在 Suno 下载 MP3 后，回到创作中心点击「导入本地音乐」添加歌曲版本。',
          });
        }
        return ok(message.id, type, {});
      }),
    ...
    await waitFor(() =>
      expect(
        screen.getAllByText(/已复制到剪贴板，并已在系统浏览器打开 Suno Create/).length,
      ).toBeGreaterThan(0),
    );
    expect(screen.queryByRole('button', { name: /确认创建歌曲/ })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /检测下载/ })).not.toBeInTheDocument();
  });
  ...
  it('cancels creation-center song generation confirms without submitting', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: /生成 Suno 歌曲/ }));
    await cancelConfirmDialog('确认生成歌曲', /系统浏览器打开 Suno Create/);
    expect(generatePayloads).toHaveLength(0);

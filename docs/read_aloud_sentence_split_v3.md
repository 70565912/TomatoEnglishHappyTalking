...
- 输出只切分、不改写，不跨文章或章节边界。普通内容仍按段落求解；完整匹配引语若因诗歌、菜单或
  排版换行跨越空行，会在朗读求解层合并涉及的段落和 parser sentence，内部布局空白折叠为一个
  空格，不能再用段落或正字句边界强制切碎同一段对话。
- 匹配直/弯双引号内 `≤16` 词引语禁止内部切分；17–20 词仍按普通合格标点规则；>20 词优先
  引号外、外部不足时才在引语内部切分。右引号留在左块，提示语不得成为无意义短尾。
- 直双引号的开、关角色按前后字符和标点语境识别，不按奇偶次序机械交替；同形嵌套用栈配对，
  最内层完整范围控制短引语硬阻断。未闭合开引号可短暂跨空行，但下一段首个引号若是新的开引号，
  必须放弃旧状态；所有未闭合引号进入全书专项审计。
- 不超过 16 词的原句不因长度切分；17–20 词原句开始搜索全部原文标点，合格标点路径优先于
  KEEP，但不启用无标点切点。
- 21–30 词先求完整标点路径；标点路径仍留下超过 20 词的长段时，继续建立安全语法切点。
- 所有原文逗号均进入候选识别，再分为从句逗号、短语逗号和歧义逗号；不能因为 parser 未确认
  从句就提前删除一个原文标点位置。
- 纯标点无解时建立所有原文安全词间间隙组成的完整候选格；字符/token 内部、多词 token
  内部、数字/小数/缩写内部、clitic 和 offset 异常才是硬阻断。
- UD 子树、跨越弧和 `det/case/aux/cop/mark/cc/compound/compound:prt/fixed/flat/amod/nsubj/csubj/obj/iobj/nummod/nmod:poss`
...
完整路径使用严格字典序比较：30/20 硬约束和原文句界之后，先排除无意义的 1–5 词碎片，
再依次消除 28–30、25–27、21–24 词长段；受保护 crossing、边界自然度随后判断，合格标点再推动
17–20 词继续切分；同级方案再优先引号外切点。短片段、8–16 偏离、切分次数和长度均衡最后。`8/22/7` 必须优于
`8/29`；最少切点不得压过字幕显示和朗读负担。UDPipe crossing 是必须复核的软证据，不得因
解析错树让 `1/30/2` 压过可读的 `16/17`。
...
- 同一批 243 项的直接基线为退役 V2 精确匹配 112 项（46.09%）。项目 EWT 模型配合历史 V3.3
  确定性流程精确命中 207 项（85.19%），且 243/243 的人工认可路径都存在于候选集；十类困难
  输入为 10/10，冻结 EWT 留出集为 60/60。V3.5 已改变历史 KEEP 政策，必须重新审核相关金标，
  不能把旧精确匹配率直接当作当前质量结论。候选召回达标不等于所有 AI 厂家已经达到发布标准。
...
- 所有输出必须完整回拼、不丢词重词、不跨文章或章节硬边界；除完整匹配引语的跨空行布局合并及
  其内部 parser sentence 合并外，不跨普通段落或正字句；每段和最长无标点段都不超过 30 词；
  17–20 词的合格标点切点必须参与择优，21–30 词标点路径仍超长时允许补充安全语法切点，
  受保护依存关系必须零违规。

...
- 用户可在审核框中手动修改章节描述和分镜描述，或设置场景数量后确认「按此数量匹配分镜」（`refreshPromptReview` + `targetSceneCount`，AI 路径仍 ≤12）显式再次调用文本 AI；也可点击“自动生成章节规划”刷新（AI 规划硬顶仍为 12）。QA 可用 `pictureBook.replaceChapterPlan` 整表重设分镜（含句子区间），不调文本 AI；手工/QA 分镜**可以超过 12**，并同步页骨架供本地导入。
- `pictureBook.savePromptReview` 只保存当前可见草稿，不调用图片 API、不删除旧图；`pictureBook.confirmPromptReview` 才提交可见审核内容并触发顺序组图（改景数后确认仍整章删旧重生）。若当前分镜 **>12**，确认组图必须明确拒绝（万相连续组图上限），不得静默截断；本地 `importPageImage` 不受此限制。
...
- 场景切分按通用「插画情况」三轴，不按文学微节拍：先把直接引语、对话、歌词、喊话和内心独白转成叙事事件，再按原始句子顺序构造 scene。同一插画情况 = 地点/时间、主视觉焦点人物组、中心进行中活动三者连续。中心活动指焦点人物的主任务及目标，不是每个新可见节拍、道具、姿态或台词。只有至少一轴实质变化到同一张插画无法同时表达前后内容时才开新 scene；人物焦点变化不以是否入场/离场为必要条件。不要因为句子边界、对话轮次、提问、回答、谜语、争论、评价、玩笑、反应、情绪、旁白或同类型重复微动作单独切分；同一事故的原因、直接结果和立即收拾保持同景。连续事实、例子、列表项或一般陈述若属于同一主题与时间/地点框架，应作为同一主题块用一张 montage 表达，不得一事实一景；该局部事实块规则不能覆盖顺序移动、物件操作、发现、事故或其它因果动作。编号句子只是覆盖锚点，一张图可覆盖很多句；候选边界若只差在说话/反应/情绪/余波收拾则合并进周围 scene。每个 `sceneDescription` 只能使用自身句子区间内的事件并保持人物动作归属，返回前逐对审核相邻边界。`chapterDescription` / `sceneDescription` 写入转写后的可见动作、反应、物件和状态，禁止原句/引号台词与谜面原文，也禁止只用 `exchange`、`conversation`、`discuss`、`debate` 等空洞 meta 词而不写清发生了什么。不要在规则里写死茶桌/厨房/法庭等场景名当合并条件。**AI 规划 / AI 组图**每个 scene 对应一张图片并按顺序覆盖完整句子范围，最多 12 个 scene；scene 数跟随插画情况变化，不要为凑满上限而拆。新生成的 AI 计划若超出上限、索引/区间不连续或未完整覆盖句子槽，直接判为无效并提示重试，不再静默截断或扩张最后一景；旧持久化计划不受影响。手工/QA `replaceChapterPlan` 与本地导入可超过 12 景；`confirmPromptReview` 在 >12 时明确拒绝出图。
...
调优记录：2026-07-20 起，章节分镜切分改为通用「插画情况」三轴（地点/时间、主视觉焦点人物组、中心进行中活动），并明确中心活动 = 焦点人物的主任务及目标而非每个可见节拍；同类型重复微动作与同一事故的直接结果/收拾余波不单独开景；删除茶桌/厨房/路边等场景专名合并句；禁止「多张一句再 dump 到最后一景」。复审 v4 进一步增加区间内人物动作归属、相邻边界终检和新 AI 响应严格结构校验。v5 不再让模型先判断整篇体裁，只为同一主题/时间地点框架内的连续事实或列表增加局部 montage 规则，并明确不能覆盖叙事和流程动作；最终旧文章矩阵 id 46/68/69/91 分别为 4/4/4、8/8/9、3/3/3、9/9/9 景，12 次结构检查全过。对话转叙事等已调优约束保留。旧 `story_chapters.summary_json` 不自动迁移，用户刷新章节规划或重新 `article.create` 后才使用新规则。完整过程、回归踩坑与实测表见 `docs/picture_book_chapter_plan_scene_split_tuning.md`。

更早（2026-07-12）起，章节规划从“删除对话内容”改为“把对话/喊话/歌词/内心独白转成第三人称可见画面叙事”，保留情节与场景信息，同时禁止引号台词、气泡文案和谜面/歌词原文。现场探针后进一步要求：优先写动作/姿态/物件/空间关系，少用 ask/explain/tell/reply/say 串场。E22 茶桌段回归重点：酒、礼貌、个人评价、谜语、反驳和沉默应写入可见叙事，但不能拆成多张语义 scene，也不能只写 `exchange` / `conversation` / `riddle` / `remark` 等空洞 meta。此前（2026-07-08）旧版 `compact`、`smallest complete scene set`、弱边界合并和叙述微阶段合并规则已不再作为正式章节规划策略。

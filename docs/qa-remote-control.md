...
- 创作中心覆盖绘本、歌曲生成（Suno / 阿里云百聆 / ElevenLabs Music）和视频导出面板，且不再展示 MiniMax API。
- 练习中心只展示跟读/对话入口，不混入连续听力或创作按钮。
- 设置页按文本处理、图片生成、语音合成、音乐生成四个能力分区展示 provider；语音合成可切换阿里云、火山、ElevenLabs，并能显示在线声音列表状态；歌曲生成可切换 Suno、阿里云百聆、ElevenLabs Music，不再展示 MiniMax。
...
- 创作中心：绘本、歌曲生成、视频导出面板可切换，歌曲生成可识别 Suno / 阿里云百聆 / ElevenLabs Music，不出现 MiniMax API 或旧歌曲来源。
- 练习中心：只提供跟读和对话入口，不出现绘本/歌曲/视频生产按钮。
- 设置：文本处理、图片生成、语音合成、音乐生成四个能力分区可见；ElevenLabs 声音列表、Suno 输出目录和相关超时设置可保存；不出现 MiniMax 或默认歌曲来源。

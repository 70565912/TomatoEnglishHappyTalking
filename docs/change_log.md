...
- **发布 v1.8.3**：`app/pubspec.yaml` → `1.8.3+13`。补丁保留 `syntax_solver_v3_11`，补上 Windows 窗口位置与最大化状态，以及播放/导出共用的绘本页转场。
- **Windows 窗口**：关闭时把正常位置、大小和最大化状态写入当前用户注册表，下次启动恢复；过小的窗口矩形不保存。
- **绘本转场**：听力内联、听力全屏、歌曲内联和歌曲全屏读取录制设置 `pageTransition`。设置变更立即写入本地配置，启动时载入。播放与导出共用 `PageTransitionRenderer`，用 `display` 图生成 `1280x720`、8 帧、500ms 的 PNG；`pageCurl` 为正式转场。帧落后时可以跳帧，但不阻塞音频和字幕，最后必须停在目标页。`none` 或 `prefers-reduced-motion` 时瞬间切换。
- **分句测试**：补上尾随后置修饰语逗号切分，并合并 E13 回归夹具。求解器版本不变，已保存文章不自动重分句。
- 发布说明见 [v1.8.3](releases/v1.8.3.md)。

- **历史 Release 更新记录补齐**：根据版本标签补录 v1.0.0、v1.1.0、v1.3.0、v1.4.0、v1.7.0、v1.8.1 的功能更新记录；保留原附件、标签和发布日期。发布脚本改为上传对应版本文档全文，缺失或空白时在创建标签前报错，避免再次只发布下载模板。

- **v1.8.2 / 分句V3.11**：应用版本`1.8.2+12`，求解器`syntax_solver_v3_11`。不足4词源单元先合并，可在同一文章内跨普通段落；恰好4词可保留。合并后每单元一次事实构建、一次DAG，保留原长度分档和30词硬上限。
- **去除重复处理**：删除旧1词后置合并和29/2借词逻辑，所有候选边至少4词；连续短链利用原16词舒适上缘分组，避免无必要的大DAG。生产代码净减少132行。
- **验证**：212项相关回归及Windows原生集成通过；154章同源回放短块308→0，W外边界变化0。代表章7对交替AOT中位耗时下降29.7%，密集短链压力测试无回退。已有文章读取、重启后保持原分句。
- 发布构建及完整回归结果见[发布说明](releases/v1.8.2.md)，算法证据见[验证记录](read_aloud_short_merge_validation_20260922.md)。
...
- **23 个候选路径门禁归一**：9 个违反当前长度/损坏正文规则的旧期望按统一规范重新裁决；其余
  14 组改用局部审核单元和同一 DAG coverage 证明规则可达，不扩大 `K=24`，不增加书籍白名单、
  固定路径保送或第二次 DP。243 项结构金标中另有11条历史完整路径按长度、标点和紧密成分规则
  逐项重审并记录原因，不以当前首选反向生成期望；55 个审核单元与 243 项结构金标全部通过。
...
- **发布 v1.7.0**：`app/pubspec.yaml` → `1.7.0+9`。英文朗读分句内部求解器定版为
  `syntax_solver_v3_9`（持久化协议仍为 `read_aloud_dp_v3` / `reviewed_dp_v3`）。
- **质量门禁**：相对 DB 的不支持切法裁决硬错误 0；相对已发布 `syntax_solver_v3_8`
  （r8n16）完成 Alice/Willows 全量对照与嫌疑切点裁决；短语动词误解析切点 `go|on` 等已收窄修复。
- **性能**：同机 AOT 交替回放中，代表章与 Alice+Willows 全量相对 v3.8 中位耗时约
  −4%～−31%（章级求解器）与全量约 −22%（端到端 wall），无回退。
- **结构**：生产分句五件套相对 v3.8 净增约 713 行（可解释机制增量）；事实/格点/求解仍为唯一链路。
- **产品说明**：已发布文章仍使用持久化句槽，不会因升级自动重分；本仓库另以离线迁移显式更新
  Alice/Willows 句槽与听力媒资（不经正式 App 静默重分）。
...
- **固定语料零不支持变化**：Alice 39 章与 Willows 62 章共 101 章、2,927 个正字句完成门禁；
  相对最近接受基线的 27 个变化原句全部通过统一规范审核，不支持变化 0、未分类变化 0。
  最终 7,967 个朗读句段原文回拼失败 0、超过 30 词 0，最长句段 27 词。
- **原生解析与认可路径闭环**：Windows 原生 243 项评测中人工认可路径覆盖 243/243，解析器保持
  健康，没有项目落入紧急原句输出；生产链路不依赖历史兼容求解器或并行候选管线。
- **性能与发布行为**：同机、同冻结输入的 AOT 交替回放中，三个代表章节相对重构前实现的中位
  耗时降低约 70%–92%。已发布文章继续使用持久化句槽，不会自动重分；新建文章或用户显式重建
  后才应用新求解器。
- **公开说明同步**：中英文 README 在首屏增加 v1.6.0 最新版本摘要，并链接生产整合报告、统一
  规范和测试评测索引，便于用户直接了解本次版本的效果、边界和复现证据。
- **Android 字体打包修复**：按 Flutter 3.41.9 官方应用模板补充 `cupertino_icons` 直接依赖，
  确保依赖链引用的 Cupertino 图标字体进入正式 APK，消除 Release 构建的字体缺失提示。

## 2026-08-11

- **发布 v1.5.0**：`app/pubspec.yaml` → `1.5.0+7`。英文朗读分句内部求解器升级为
  `syntax_solver_v3_6`，完整匹配引语进入全局上下文求解；不再把 UDPipe 的每个正字句强制
  变成朗读边界。
- **引语分句可读性修复**：完整引语 `≤16` 词时禁止内部切分；`17–20` 词继续按普通长度
  压力积极使用合格标点；`>20` 词时先比较引语外切点，引语内部切点只在外部方案不足时
  使用，仍保留 30 词硬上限。右引号前后的边界优先于引语内部边界，右引号保留在左块，
  提示语不得制造无意义短尾。
- **候选审计与回归覆盖**：候选 JSON 新增引语范围、引语词数、引号边缘和阻断/降级原因；
  增加短重复喊话、同一引号内多句、长引语、直/弯引号、相邻引语、提示语归属、未闭合引号
  与诗歌引号测试，并保留 E01、E61、全文金标、holdout 和原生 UDPipe 评测。
- **一次性迁移与正式产品隔离**：全书审计、译文迁移、图片区间重映射、音视频快照和验证只保留
  在离线 `tools/`；正式 WebShell、数据库服务和 Bridge 不携带一次性全书迁移入口。该约束已写入
  `AGENTS.md`。
- **历史音频复用增强**：听力材料按保守英文朗读词序匹配历史缓存，纯标点或空格变化可以复用，
  撇号和连字符仍作为词内语义保留；新增测试防止重复调用远程 TTS。
- **语音引擎特例归档**：E02 连续食物名词串作为 TTS 模型选择评测样例归档；当前火山 Seed TTS
  2.0 实测可以正确处理，其他引擎接入前必须单独复测。

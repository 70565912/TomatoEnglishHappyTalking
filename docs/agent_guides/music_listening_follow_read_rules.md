...
- 听力普通内联、听力全屏、歌曲内联和歌曲全屏切绘本页时统一读取录制设置 `pageTransition`（`none` / `crossFade` / `panZoomFade` / `slide` / `pageCurl`）。转场设置由录制设置框改变时即时写入本地配置，应用启动时由 `recording.settings.load` 统一载入；取消设置框也保留已选择的转场。播放与导出共用 Flutter `PageTransitionRenderer`，播放通过 `pictureBook.transitionFrames` 使用 `display` 图片生成固定 `1280x720`、8 帧、`500ms` 的 PNG 批次，不使用 CSS 近似、`clip-path` 或 `transitionend`。帧生成和绘制不阻塞音频或字幕时间轴，性能不足时允许跳到最新帧，但必须最终显示目标页；转场生成失败也立即落到目标页。`prefers-reduced-motion` 或 `none` 时瞬间切换。听力全屏启动必须传入当前选中句 `startIndex`，禁止写死从 0 重播。

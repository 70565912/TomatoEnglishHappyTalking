...
parser 原始精度继续由独立 EWT test split 评估；分句结果由243项任务金标和 Alice＋Willows
101章全量回放评估。两部分不能混成同一排行榜。
...
- 243 项朗读分句金标与 Alice＋Willows 101章全量回放，用于生产模型和候选求解器的任务级验收。
...
- 历史冻结 EWT 留出集曾为60/60；该短句集现已退出分句发布门禁；
- 十类困难输入候选覆盖 10/10；
